...
def per_block_int8(
    ...
    _fused.quant_per_block_int8_cuda(q, q_int8, q_scale, sm_scale, BLKQ, _tensor_layout, actual_head_dim)
    if km is not None:
        km = km.squeeze(1) if _tensor_layout == 0 else km.squeeze(2)
        _fused.quant_per_block_int8_fuse_sub_mean_cuda(k, km, k_int8, k_scale, BLKK, _tensor_layout, actual_head_dim)
    else:
        _fused.quant_per_block_int8_cuda(k, k_int8, k_scale, BLKK, _tensor_layout, actual_head_dim)
...
def per_warp_int8(
    ...
    _fused.quant_per_warp_int8_cuda(q, q_int8, q_scale, BLKQ, WARPQ, _tensor_layout, actual_head_dim)
...
def sub_mean(
    ...
    v_smoothed = torch.empty(v.shape, dtype=v.dtype, device=v.device)
